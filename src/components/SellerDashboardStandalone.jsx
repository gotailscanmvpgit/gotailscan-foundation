...
export default function SellerDashboardStandalone() {
    ...
    try {
      ...
      if (session) {
        ...
        localStorage.setItem(
          "demo_searches",
          JSON.stringify([newRecord, ...demoHistory]),
        );
      }
    } catch (err) {
  ...
  const getPriceShield = () => {
    ...
    if (result.forensic_records?.ntsb_count === 0)
      shields.push({
        label: "Zero Accident History",
        icon: "✓",
        color: "emerald",
      });
    if (!result.forensic_records?.liens_found)
      shields.push({ label: "Clear Title", icon: "✓", color: "emerald" });
    if (result.logbook_audit?.findings?.continuity_score > 80)
      shields.push({ label: "Complete Logbooks", icon: "✓", color: "emerald" });
    if (result.dormancy_analysis?.dormancy_risk === "LOW")
      shields.push({ label: "Actively Flown", icon: "✓", color: "emerald" });
    if (result.avionics_audit?.modernity_score > 70)
      shields.push({ label: "Modern Avionics", icon: "✓", color: "blue" });
    if (result.hangar_queen_index < 30)
      shields.push({ label: "Low Corrosion Risk", icon: "✓", color: "blue" });
    if (result.compliance_audit?.status === "CLEAR")
      shields.push({ label: "Clean Sanctions", icon: "✓", color: "blue" });
  ...
  const getMaintenanceAlpha = () => {
    ...
    const maintenanceEvents =
      result.logbook_audit?.findings?.maintenance_event_count || 1; // Avoid division by zero

    const hoursPerEvent = totalHours / maintenanceEvents;
    const alpha =
      ((hoursPerEvent - FLEET_MEAN_HOURS_PER_EVENT) /
        FLEET_MEAN_HOURS_PER_EVENT) *
      100;

    let rating = "AVERAGE";
    if (alpha > 20)
      rating = "EXCELLENT"; // Well-maintained, above fleet average
    ...
    return {
      ...
      rating,
      comparison,
      hoursPerEvent: hoursPerEvent.toFixed(0),
    };
  };
  ...
  const getCertifiedValueBadge = () => {
    ...
    const adGapCount =
      result.logbook_audit?.findings?.ad_gap_count ||
      result.logbook_audit?.findings?.gaps?.filter((g) =>
        g.flag?.includes("AD"),
      )?.length ||
      0;
    ...
    if (adGapCount === 0) {
      return {
        ...
        confidence: "HIGH",
        message:
          "This aircraft has zero Airworthiness Directive gaps, qualifying for premium market positioning.",
      };
    }
  ...
  const getBuyerDemand = () => {
    ...
    return {
      ...
      priceMovement:
        baseInterest > 60 ? "+3.2%" : baseInterest > 40 ? "+1.1%" : "-0.5%",
    };
  };
  ...
  useEffect(() => {
    ...
    if (!isCleanMakeModel(aircraftData.make_model)) {
      console.log(
        "[Seller Dashboard] Resolving unclear make/model:",
    ...
    <div className="cockpit-container">
        ...
        <div className="fixed inset-0 z-[200] flex flex-col items-center justify-center bg-black/90 backdrop-blur-2xl animate-fade-in transition-all">
          <div className="relative flex flex-col items-center">
            ...
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-black text-blue-500 tracking-[0.4em] uppercase opacity-60">
          ...
          >
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                >
                  Your aircraft scores{" "}
                  {alpha.score > 65
                    ? "significantly above"
                    : alpha.score > 50
                      ? "above"
                      : "at"}{" "}
                  market average based on forensic markers and maintenance
                  efficiency.
                </motion.div>
              </div>
            </div>
            ...
            >
                    ...
                    {shields.map((shield, i) => (
                      <div
                        ...
                        style={{
                          ...
                          borderRadius: "8px",
                          background:
                            shield.color === "emerald"
                              ? "rgba(16, 185, 129, 0.1)"
                              : "rgba(59, 130, 246, 0.1)",
                          border: `1px solid ${shield.color === "emerald" ? "rgba(16, 185, 129, 0.2)" : "rgba(59, 130, 246, 0.2)"}`,
                        }}
                      >
                        <div
                          style={{
              ...
              <div className={internalCardClass}>
                  ...
                  >
                    <div
                      style={{
                        ...
                        fontWeight: "900",
                        color:
                          maintenanceAlpha.alpha > 0 ? "#10b981" : "#ef4444",
                      }}
                    >
                      {maintenanceAlpha.alpha > 0 ? "+" : ""}
                      {maintenanceAlpha.alpha}%
                    </div>
                    <div
                      style={{
                        padding: "4px 12px",
                        borderRadius: "6px",
                        background:
                          maintenanceAlpha.rating === "EXCELLENT"
                            ? "#10b981"
                            : maintenanceAlpha.rating === "GOOD"
                              ? "#3b82f6"
                              : "#eab308",
                        color: "black",
                        fontSize: "11px",
                ...
                >
                  <strong style={{ color: "#3b82f6" }}>Market Insight:</strong>{" "}
                  Aircraft with positive alpha demonstrate superior maintenance
                  efficiency, commanding premium pricing in competitive markets.
                </div>
              </div>
              ...
              {result.predictive_maintenance && (
                <div
                  className={`${internalCardClass} mb-6`}
                  style={{
                    borderLeft: `4px solid ${result.predictive_maintenance.pag_score > 80
                        ? "#ef4444"
                        : result.predictive_maintenance.pag_score > 50
                          ? "#f59e0b"
                          : "#10b981"
                      }`,
                  }}
                >
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <TrendingUp
                        className={
                          result.predictive_maintenance.pag_score > 80
                            ? "text-red-500"
                            : "text-amber-500"
                        }
                        size={24}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    ...
                    <div className="flex flex-col items-center justify-center p-4 bg-black/40 rounded-lg border border-white/5">
                      <div className="text-[10px] text-gray-500 font-bold uppercase mb-2">
                        prob. aircraft grounding
                      </div>
                      <div
                        className={`text-6xl font-black mb-1 ${result.predictive_maintenance.pag_score > 80
                            ? "text-red-500"
                            : result.predictive_maintenance.pag_score > 50
                              ? "text-amber-500"
                              : "text-emerald-500"
                      ...
                      <div className="text-[11px] font-bold text-white uppercase tracking-widest bg-white/10 px-3 py-1 rounded">
                        PAG SCORE
                      </div>
                    </div>
                    ...
                    <div className="md:col-span-2 space-y-4">
                      <div
                        className={`p-4 rounded-md border ${result.predictive_maintenance.pag_score > 80
                      ...
                      >
                        ...
                        <div className="text-lg font-black text-white">
                          {result.predictive_maintenance.advisory}
                        </div>
                        <div className="text-[11px] mt-1 opacity-80 font-mono">
                          {result.predictive_maintenance.system_type} •{" "}
                          {result.predictive_maintenance.forecast?.length || 0}{" "}
                        ...
                        {result.predictive_maintenance.forecast
                          ...
                          .map((item, idx) => (
                            ...
                            >
                              <div className="flex items-center gap-3">
                                <div
                                  className={`w-1.5 h-1.5 rounded-full ${item.status === "URGENT"
                                      ? "bg-red-500 animate-pulse"
                                      : "bg-amber-500"
                                    }`}
                                ></div>
                                <div>
                                  <div className="text-[11px] font-bold text-gray-200 uppercase">
                                    {item.part}
                                  </div>
                              ...
                              <div className="text-right">
                                ...
                                <div className="w-16 h-1 bg-gray-700 rounded-full mt-1 overflow-hidden">
                                  <div
                                    className={`h-full ${item.health_pct < 20
                                        ? "bg-red-500"
                                        : "bg-emerald-500"
                                      }`}
              ...
              {result.predictive_maintenance && (
                <div
                  style={{
                    ...cardStyle,
                    background:
                      "linear-gradient(135deg, rgba(30, 58, 138, 0.2), rgba(0, 0, 0, 0.4))",
                    border: "1px solid rgba(59, 130, 246, 0.3)",
                ...
                >
                  ...
                  >
                    ...
                    >
                      <strong>Justify Your Premium:</strong> Use this predictive
                      report to prove your aircraft is statistically "Lower
                      Risk" than the current fleet average. By showing no
                      imminent major component failures in the next 200 hours,
                      you can confidently defend a price point{" "}
                      <strong>3-5% above bluebook</strong> as a turn-key asset.
                    </div>
                  </div>
                </div>
              )}
                      ...
                      >
                        ...
                        <div
                          style={{
                            fontSize: "20px",
                            fontWeight: "900",
                            color: demand.priceMovement.startsWith("+")
                              ? "#10b981"
                              : "#ef4444",
                          }}
                        >
                          {demand.priceMovement}
                        </div>
                      </div>
                    </div>
                  </div>
      ...
      <HangarDoorModal
        ...
        searchHistory={
          !session && localStorage.getItem("demo_mode") !== "true"
            ? guestHistory
            : localStorage.getItem("demo_searches")
              ? JSON.parse(localStorage.getItem("demo_searches")).map(
                (s) => s.tail_number,
              )
              : []
        }
